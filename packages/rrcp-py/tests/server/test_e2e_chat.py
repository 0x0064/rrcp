...
async def test_full_chat_scenario(clean_db: asyncpg.Pool) -> None:
    ...
    resp = await alice_client.post("/acp/threads", json={"tenant": {"org": "A"}})
    assert resp.status_code == 201
    thread_id = resp.json()["id"]

    resp = await alice_client.get("/acp/threads")
    assert thread_id in [t["id"] for t in resp.json()["items"]]

    resp = await bob_client.get(f"/acp/threads/{thread_id}/events")
    ...
    resp = await alice_client.delete(f"/acp/threads/{thread_id}/members/u_bob")
    assert resp.status_code == 204
    resp = await bob_client.get(f"/acp/threads/{thread_id}/events")
    assert resp.status_code == 403

    resp = await alice_client.delete(f"/acp/threads/{thread_id}")
    assert resp.status_code == 204
    resp = await alice_client.get(f"/acp/threads/{thread_id}")
