...
async def test_add_and_list_members(store: PostgresThreadStore) -> None:
...
async def test_is_member(store: PostgresThreadStore) -> None:
    user = UserIdentity(id="u1", name="Alice")
...
async def test_remove_member(store: PostgresThreadStore) -> None:
    user = UserIdentity(id="u1", name="Alice")
    await store.add_member("th_1", user, added_by=user)
...
async def test_add_member_idempotent(store: PostgresThreadStore) -> None:
    user = UserIdentity(id="u1", name="Alice")
    await store.add_member("th_1", user, added_by=user)
    await store.add_member("th_1", user, added_by=user)
    members = await store.list_members("th_1")
