...
async def store(clean_db: asyncpg.Pool) -> PostgresThreadStore:
    return PostgresThreadStore(pool=clean_db)
...
async def test_create_and_get(store: PostgresThreadStore) -> None:
...
async def test_get_missing_returns_none(store: PostgresThreadStore) -> None:
    assert await store.get_thread("nope") is None


async def test_update_thread_tenant(store: PostgresThreadStore) -> None:
...
async def test_update_thread_metadata_only(store: PostgresThreadStore) -> None:
    await store.create_thread(_new_thread(tenant={"org": "A"}))
...
async def test_delete_thread(store: PostgresThreadStore) -> None:
